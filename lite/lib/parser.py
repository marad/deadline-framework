...
def parseType(key,typ) :
	...
	repeats = []
	while typ :
		c = matchingBracket(typ[-1])
		if not c : break
		i = typ.rfind(c)
		j = len(typ)-1
		sepNL = (c=='<')
		for expr in reversed(typ[i+1:j].split('|')) :
			repeats.append( (expr,c=='<') )
		typ = typ[0:i]
	...
	for expr,sepNL in repeats :
		callfunc = "repeatStrip(iter_,{},int({}),{})".format(callfunc, expr, sepNL)
